angular.module('ViewCtrl', []).controller('ViewController', function($scope, SensorsService, fileUpload, $q) {
	$scope.responseData = [];	
    ...
    //     var x = parseInt(evt.x - (offsetX + width/2), 10);
    // 	var y = parseInt(evt.y - (offsetY + height/2), 10);
    // 	data.pos = {
    // 		x: x,
    // 		y: y
    // 	};	
    ...
    $scope.onClick = function(zone) {    	
    	var img = $scope.stage.findOne('#'+zone.id);

  //   	console.log(img.image().src);
		img.image().src = 'img/square-stripped.png';
