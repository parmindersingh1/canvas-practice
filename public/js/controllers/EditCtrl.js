angular.module('EditCtrl', []).controller('EditController', function($scope, SensorsService, fileUpload, $q) {
	$scope.responseData = [];
	$scope.sensors = SensorsService.sensors;
	// $scope.zones = 	SensorsService.getZones();
	...
	$scope.floorData = SensorsService.getZones();
	console.log($scope.floorData);
	$scope.responseData = $scope.floorData.zones;
    ...
    $scope.onDropComplete=function(data,evt){
    	...
    	$scope.responseData.push(data);
    	  // get the drop payload (here the payload is the image)
		    ...
		    console.log(imgSrc, data, centerX, centerY);
    ...
    $scope.addImageOnCanvas = function(data, imgSrc) {
    		var x = data.pos.x;
    		var y = data.pos.y;
    ...
    $scope.removeSensors = function(dataObj) {
    	_.remove($scope.sensors, {id: dataObj.id});
    }
    ...
    $scope.$on('KONVA:READY', function kineticReady (event, stage) {
		...
		_.forEach($scope.responseData, function(zone) {
			$scope.addImageOnCanvas(zone, $scope.floorData.url);
			$scope.removeSensors(zone);
		});
    ...
    $scope.save = function() {
    	...
    	localStorage.setItem('floorData',JSON.stringify($scope.floorData));
    }
});